...
export function useEdgarCompany() {
  ...
  const [history, setHistory] = useState<Record<string, FinancialHistoryResponse>>({});
  ...
  const fetchFilings = useCallback(async (ticker: string) => {
  ...
  const fetchHistory = useCallback(async (ticker: string, metric: FinancialMetric, quarters: number = 8) => {
