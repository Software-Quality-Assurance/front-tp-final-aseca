...
export default function ExploreScreen() {
  ...
  const renderContent = () => {
    if (selectedTicker) {
      const isLoading =
        companyHook.loadingMetrics || companyHook.loadingFilings;
      ...
      return (
        <View className="flex-1">
            ...
            <ExploreLinkPressable onPress={handleBack}>
              <ExploreLinkButton>
                ...
                >
                  ...
                  <ThemedText className="ml-1 font-medium" themeColor="text">
                    Back to search
                  </ThemedText>
                </View>
              </ExploreLinkButton>
          ...
          {isLoading ? (
            ...
            <View>
              {companyHook.metrics && (
                <CompanyMetricsCard metrics={companyHook.metrics} />
              )}

              <View
                className="flex-row flex-wrap gap-2 mb-2"
                testID="metric-selectors"
              >
                {(
                  [
                    'REVENUE',
                  ...
                  >
                    <ThemedText
                      className={`text-xs font-semibold ${selectedMetric === m ? 'text-white' : 'text-gray-900 dark:text-gray-100'}`}
                    >
                      {m.replace('_', ' ')}
                    </ThemedText>
                  </TouchableOpacity>
              ...
              {companyHook.loadingHistory ? (
                ...
                companyHook.history[selectedMetric] && (
                  <CompanyHistoryChart
                    history={companyHook.history[selectedMetric]}
                  />
                )
              )}

              {companyHook.filings && (
                <CompanyFilingsCard data={companyHook.filings} />
              )}
            </View>
          )}
        </View>
      ...
      <View className="flex-1">
        <ExploreTitleSection>
          ...
          <ThemedText className="mb-4" themeColor="textSecondary">
            Search for an SEC EDGAR registered company to analyze its financial
            health.
          </ThemedText>
        </ExploreTitleSection>
        ...
        {searchHook.error ? (
          <ThemedText testID="search-error" className="text-red-500 mt-2">
            {searchHook.error}
          </ThemedText>
        ) : null}
        ...
        {!searchHook.loading &&
          ...
          searchHook.results.length === 0 && (
            <View
              ...
              className="items-center justify-center py-10 opacity-50"
            >
  ...
  return (
    <ExploreScroll testID="explore-screen">
      <ExploreContainer>{renderContent()}</ExploreContainer>
    </ExploreScroll>
  );
}
