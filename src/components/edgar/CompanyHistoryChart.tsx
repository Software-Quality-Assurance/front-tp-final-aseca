...
export function CompanyHistoryChart({ history }: Props) {
  if (history.points.length === 0) {
    return (
      <View className="bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4 mb-4">
         <ThemedText className="text-sm" themeColor="textSecondary">No historical data available.</ThemedText>
      </View>
    )
  }
  ...
  return (
    <View className="bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4 mb-4" testID="company-history-card">
      <View className="flex-row justify-between items-center mb-3">
        <ThemedText className="font-bold text-lg">History ({history.metric})</ThemedText>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View className="flex-row gap-4 items-end pt-4 pb-2">
          {history.points.map((point, index) => {
            const heightPercent = Math.max(10, Math.min(100, (Math.abs(point.value) / maxValue) * 100));
            const isNegative = point.value < 0;
            
            return (
              <View key={`${point.period}-${index}`} className="items-center" testID={`history-point-${point.period}`}>
