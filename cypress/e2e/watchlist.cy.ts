...
describe('Watchlist — Feature 6', () => {
  ...
  beforeEach(() => {
    ...
    cy.visit('/watchlist');
    cy.contains('Watchlist', { timeout: 10000 }).should('be.visible');
  });

  it('6.1 — watchlist vacío muestra estado vacío', () => {
    cy.get('[data-testid="watchlist-empty-state"]', { timeout: 10000 }).should('be.visible');
    cy.contains('Your watchlist is empty').should('be.visible');
  });

  it('6.1 — agregar compañía a la watchlist', () => {
    cy.intercept('POST', endpoints.watchlist.ticker(testTicker)).as('addToWatchlist');
    
    cy.get('[data-testid="add-watchlist-button"]').click({ force: true });
    cy.get('[data-testid="add-watchlist-modal"]').should('be.visible');
    
    cy.get('[data-testid="add-watchlist-ticker-input"]').type(testTicker, { force: true });
    ...
    cy.get('[data-testid="add-watchlist-modal"]', { timeout: 10000 }).should('not.exist');
    cy.contains(testTicker, { timeout: 10000 }).should('be.visible');
  });

  it('6.1 — intentar agregar duplicado retorna error 409', () => {
    // Primero nos aseguramos que esté agregada
    cy.request({
      method: 'POST',
    ...
    cy.get('[data-testid="add-watchlist-error"]').should('contain', 'already in watchlist');
  });

  it('6.1 — intentar agregar ticker inexistente retorna error 404', () => {
    const invalidTicker = 'INVALID' + uniqueId;
    cy.get('[data-testid="add-watchlist-button"]').click({ force: true });
    cy.get('[data-testid="add-watchlist-ticker-input"]').type(invalidTicker, { force: true });
    cy.get('[data-testid="add-watchlist-submit-button"]').click({ force: true });
  ...
  it('6.1 — eliminar compañía de la watchlist', () => {
    ...
    // Confirmación de Alert (Cypress maneja windows alerts automáticamente pero aquí es un custom Alert de RN si se usa un polyfill o modal)
    // En web-mode de Expo, Alert.alert suele ser un window.confirm
