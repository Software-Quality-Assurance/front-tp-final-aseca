...
describe('Portfolio — Feature 5', () => {
  ...
  beforeEach(() => {
    ...
    cy.visit('/')
    cy.get('[data-testid="portfolio-add-button"]', { timeout: 10000 }).should('be.visible')
  })
  ...
  function buyPosition(ticker: string, quantity: string) {
    ...
    cy.get('[data-testid="add-position-ticker-input"]').clear({ force: true }).type(ticker, { force: true })
    cy.get('[data-testid="add-position-quantity-input"]').clear({ force: true }).type(quantity, { force: true })
    cy.get('[data-testid="add-position-submit-button"]').click({ force: true })
    cy.wait('@buyOp')
    cy.get('[data-testid="add-position-modal"]', { timeout: 5000 }).should('not.exist')
  }
  ...
  it('5.1 — portfolio vacío muestra estado vacío', () => {
    cy.get('[data-testid="portfolio-empty-state"]', { timeout: 10000 }).should('be.visible')
  ...
  it('5.1 — portfolio muestra compañía, ticker, cantidad y precio', () => {
    ...
    cy.get(`[data-testid="position-item-${testTicker}"]`, { timeout: 10000 }).within(() => {
  ...
  it('5.2 — botón Add abre el modal', () => {
    cy.get('[data-testid="portfolio-add-button"]').click({ force: true })
    cy.get('[data-testid="add-position-modal"]', { timeout: 5000 }).should('be.visible')
  })
  ...
  it('5.2 — BUY agrega posición y aparece en el portfolio', () => {
    buyPosition(testTicker, testQuantity)
    cy.get(`[data-testid="position-item-${testTicker}"]`, { timeout: 10000 }).should('be.visible')
  })
  ...
  it('5.2 — BUY con fecha personalizada se registra correctamente', () => {
    ...
    cy.get('[data-testid="portfolio-add-button"]').click({ force: true })
    cy.get('[data-testid="add-position-buy-button"]').click({ force: true })
    cy.get('[data-testid="add-position-ticker-input"]').type(testTicker, { force: true })
    cy.get('[data-testid="add-position-quantity-input"]').type('1', { force: true })

    cy.get('[data-testid="add-position-now-switch"]').click({ force: true })
    cy.get('[data-testid="add-position-date-input"]', { timeout: 3000 }).should('be.visible')
    cy.get('[data-testid="add-position-date-input"]').clear({ force: true }).type('2026-01-15', { force: true })
  ...
  it('5.2 — BUY con ticker inválido muestra error', () => {
    ...
    cy.get('[data-testid="portfolio-add-button"]').click({ force: true })
    cy.get('[data-testid="add-position-ticker-input"]').type('INVALIDTICKER999', { force: true })
    cy.get('[data-testid="add-position-quantity-input"]').type('1', { force: true })
    cy.get('[data-testid="add-position-submit-button"]').click({ force: true })
    cy.wait('@createOp')
    cy.get('[data-testid="add-position-error"]', { timeout: 5000 }).should('be.visible')
  ...
  it('5.2 — BUY con cantidad 0 muestra error de validación', () => {
    cy.get('[data-testid="portfolio-add-button"]').click({ force: true })
    cy.get('[data-testid="add-position-ticker-input"]').type(testTicker, { force: true })
    cy.get('[data-testid="add-position-quantity-input"]').type('0', { force: true })
    cy.get('[data-testid="add-position-submit-button"]').click({ force: true })
    cy.contains('Quantity must be a whole number', { timeout: 5000 }).should('be.visible')
  })

  it('5.2 — BUY con cantidad decimal muestra error de validación', () => {
    cy.get('[data-testid="portfolio-add-button"]').click({ force: true })
    cy.get('[data-testid="add-position-ticker-input"]').type(testTicker, { force: true })
    cy.get('[data-testid="add-position-quantity-input"]').type('2.5', { force: true })
    cy.get('[data-testid="add-position-submit-button"]').click({ force: true })
    cy.contains('Quantity must be a whole number', { timeout: 5000 }).should('be.visible')
  })
  ...
  it('5.2 — cancelar BUY no modifica el portfolio', () => {
    cy.get('[data-testid="portfolio-add-button"]').click({ force: true })
    cy.get('[data-testid="add-position-ticker-input"]').type(testTicker, { force: true })
    cy.get('[data-testid="add-position-quantity-input"]').type('10', { force: true })
  ...
  it('5.2 — SELL por cantidad exacta comprada retorna 201', () => {
    ...
    cy.get('[data-testid="portfolio-add-button"]').click({ force: true })
    cy.get('[data-testid="add-position-sell-button"]').click({ force: true })
    cy.get('[data-testid="add-position-ticker-input"]').clear({ force: true }).type(testTicker, { force: true })
    cy.get('[data-testid="add-position-quantity-input"]').clear({ force: true }).type('5', { force: true })
    cy.get('[data-testid="add-position-submit-button"]').click({ force: true })
  ...
  it('5.2 — SELL de más acciones de las disponibles muestra error', () => {
    ...
    cy.get('[data-testid="add-position-sell-button"]').click({ force: true })
    cy.get('[data-testid="add-position-ticker-input"]').clear({ force: true }).type(testTicker, { force: true })
    cy.get('[data-testid="add-position-quantity-input"]').clear({ force: true }).type('99999', { force: true })
    cy.get('[data-testid="add-position-submit-button"]').click({ force: true })
    cy.wait('@oversell')
    cy.get('[data-testid="add-position-error"]', { timeout: 5000 }).should('be.visible')
    cy.get('[data-testid="add-position-modal"]').should('be.visible')
  ...
  it('5.2 — SELL elimina posición desde botón Delete del portfolio', () => {
    ...
    cy.get(`[data-testid="delete-position-button-${testTicker}"]`).click({ force: true })
    cy.get('[data-testid="delete-position-confirm-button"]', { timeout: 5000 }).click({ force: true })
    cy.wait('@deleteOp')

    cy.get(`[data-testid="position-item-${testTicker}"]`, { timeout: 10000 }).should('not.exist')
  })
  ...
  it('5.2 — cancelar Delete no modifica el portfolio', () => {
    ...
    cy.get(`[data-testid="delete-position-button-${testTicker}"]`).click({ force: true })
    cy.get('[data-testid="delete-position-cancel-button"]').click({ force: true })
